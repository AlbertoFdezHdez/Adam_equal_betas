...
Partial historical setup output observed during the audit mentioned Python 3.9 and, at one point in environment preparation, the following versions:
...
This evidence is not sufficient to assert that every published run used exactly that combination. In particular, the environment log included attempted package changes. These versions are therefore provenance notes, not guaranteed pins.
