...
from .callbacks import Callback, LRScheduler, EarlyStopping, StoreBestModel
...
class Model(nn.Module):
    ...
    def __init__(self, predefined_model=None):
        ...
        self.use_amp = False
        self.scaler = None
        self.lr_metric = "val_loss"
        self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        if predefined_model is not None:
            self.layers = predefined_model
            self.forward = predefined_model.forward
   
    def compile(self, loss, optimizer, optimizer_params={}, metrics=None, use_amp=False):
        """
        ...
        if isinstance(optimizer, str):
            if optimizer.lower() == "adam":
                ...
                self.optimizer = optim.AdamW(self.parameters(), **optimizer_params)  
    ...
    def train_model(self, train_data,  val_data=None, num_epochs=10, verbose=True, **_ignored_kwargs):        
        """
        ...
        for epoch in range(num_epochs):  
            ...
            if verbose:
                print(f'Epoch {epoch + 1:3d}/{num_epochs}: {self.metrics} - learning rate: {actual_lr:.2E}', flush=True)
        ...
        self.metrics.reset()
        history["training_wall_time_sec"] = time.perf_counter() - wall_start
        return history
