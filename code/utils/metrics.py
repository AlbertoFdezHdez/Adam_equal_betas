...
class Metric(ABC):
    ...
    def reset_val(self):
        pass
    @abstractmethod
    def update_train(self, output, expected, *args, **kwargs):
        pass
    @abstractmethod
    def update_val(self, output, expected, *args, **kwargs):
        pass
    @abstractmethod
    def __str__(self):
        pass
...
class Metrics(Metric):
    ...
    def update_train(self, output, expected, *args, **kwargs):
        best = []
        for metric in self.metrics_list:
            best.append(metric.update_train(output, expected, *args, **kwargs))
        return any(best)
    
    def update_val(self, output, expected, *args, **kwargs):
        best = []
        for metric in self.metrics_list:
            best.append(metric.update_val(output, expected, *args, **kwargs))
        return any(best)
...
class Accuracy(Metric):
    ...
    def update_train(self, output, expected, *args, **kwargs):
        ...
        predict = self._predictions(output)
