...
from utils.datasets import load_slimpajama
from utils.datasets.slimpajama import inspect_slimpajama_bins, resolve_nanogpt_data_dir
...
EXPERIMENT_NAME = "nanogpt_slimpajama"
...
parser.add_argument("--data-dir", default=None, help="Absolute path to the external NanoGPT dataset directory")
parser.add_argument("--smoke-test", action="store_true", help="Run a tiny end-to-end validation")
args = parser.parse_args()

SMOKE_TEST = bool(args.smoke_test)
DATA_DIR = str(resolve_nanogpt_data_dir(args.data_dir))
EPOCHS = 1
LR = 3e-4
WD = 1e-4
BETAS = (float(args.beta1), float(args.beta2))
SEED = int(args.seed)
BLOCK_SIZE = 128 if SMOKE_TEST else 1024
BATCH_SIZE = 2 if SMOKE_TEST else 8
SUBSET_TRAIN_WINDOWS = 6 if SMOKE_TEST else None
SUBSET_VAL_WINDOWS = 2 if SMOKE_TEST else None
EXPERIMENT_OUTPUT_NAME = "nanogpt_slimpajama_smoke" if SMOKE_TEST else EXPERIMENT_NAME
...
def main():
    ...
    dataset_info = inspect_slimpajama_bins(DATA_DIR)
    print(f"[nanogpt_slimpajama] data_dir={dataset_info['data_dir']}", flush=True)
    print(
        f"[nanogpt_slimpajama] opened train.bin tokens={dataset_info['train_tokens']} size_bytes={dataset_info['train_bin_size_bytes']}",
        flush=True,
    )
    print(
        f"[nanogpt_slimpajama] opened val.bin tokens={dataset_info['val_tokens']} size_bytes={dataset_info['val_bin_size_bytes']}",
        flush=True,
    )

    train_loader, test_loader, vocab_size = load_slimpajama(
        batch_size=BATCH_SIZE,
        max_length=BLOCK_SIZE,
        subset_size_train=SUBSET_TRAIN_WINDOWS,
        subset_size_val=SUBSET_VAL_WINDOWS,
        num_workers=0,
        data_dir=DATA_DIR,
    )
    first_batch = next(iter(train_loader))
    print(
        f"[nanogpt_slimpajama] constructed batches x_shape={tuple(first_batch[0].shape)} y_shape={tuple(first_batch[1].shape)}",
        flush=True,
    )
    print(f"[nanogpt_slimpajama] train_batches={len(train_loader)} val_batches={len(test_loader)}", flush=True)

    model = create_nanogpt_model(vocab_size=vocab_size, config_name="smoke" if SMOKE_TEST else "gpt2-small")
    ...
    model.compile(
        ...
        optimizer_params={"lr": LR, "weight_decay": WD, "betas": BETAS, "eps": 1e-8},
        metrics=Accuracy(),
        use_amp=False,
    )
    model.clipping = True
    ...
    print("[nanogpt_slimpajama] starting training", flush=True)
    history = model.train_model(train_loader, test_loader, num_epochs=EPOCHS, verbose=True)
    checkpoint_path = RESULTS_ROOT / EXPERIMENT_OUTPUT_NAME / "checkpoints" / f"seed_{SEED}.pth"
    checkpoint_path.parent.mkdir(parents=True, exist_ok=True)
    print(f"[nanogpt_slimpajama] saving checkpoint to {checkpoint_path}", flush=True)
    model.save_model(str(checkpoint_path), verbose=False)
    print(f"[nanogpt_slimpajama] checkpoint_saved={checkpoint_path.is_file()}", flush=True)
    history = {
        "model": "NanoGPT",
        "dataset": "SlimPajama",
