...
A very short Digits pipeline checks file formats and numerical identities without running the complete experiment grid:
...
Aggregated outputs include `table1_main.csv`, `appendix_short_window.csv`, `appendix_reverse_order.csv`, and all-pair summaries. These files are generated locally and are not tracked in the repository.
...
Only load pickle files from a trusted source. The analyzer propagates the top-level historical `epochs` field into each run before removing the first three epochs; this detail is required to reproduce the reference values.
...
The optional command implements the one-sided-boundary convention. On the archived six-task results, both conventions gave exactly the same selection rates.
