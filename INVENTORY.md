...
All files listed here are current unless explicitly marked as compatibility-only. Obsolete notebooks, probe-coordinate decompositions, private logs, and intermediate exploratory scripts were intentionally not copied into the distributable.
...
| `MANUSCRIPT_CODE_MAP.md` | Result-by-result provenance and verification status. |
...
| Path | Role |
|---|---|
| `configs/experiment_protocols.json` | Exact audited protocol, including actual constructors and observable definitions. |
...
## Private source-tree classification

The following categories existed in the development workspace but are not current distributable code: old common-gradient replay experiments; deterministic 1024-coordinate calibration scripts; local-window rather than full-history log-EMA decompositions; exploratory MLP/CNN sweeps; notebooks; duplicate display scripts; stale `paper_artifacts` tables; every generated CSV/PDF; private raw results; downloaded datasets; credentials; server logs; and caches. They remain outside the release for provenance and were not deleted.
